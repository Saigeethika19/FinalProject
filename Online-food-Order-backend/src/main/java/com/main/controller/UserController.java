...
public class UserController {
	...
	public ResponseEntity<UserRegistration>createUser(@Valid @RequestBody UserRegistration user)
	{
		UserRegistration savedUser = userService.createUser(user);
		return new ResponseEntity<>(savedUser,HttpStatus.OK);
	}
	@PostMapping("/{userId}")
	public ResponseEntity<UserRegistration>createUser(@PathVariable Integer userId)
	{
		UserRegistration getUserById = userService.getUserByID(userId);
		return new ResponseEntity<UserRegistration>(getUserById,HttpStatus.OK);
	}
